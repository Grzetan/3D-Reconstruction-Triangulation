...
std::vector<std::pair<cv::Mat, cv::Mat>> getData(const std::string& path) {
  ...
  while (std::getline(file, line)) {
    ...
    cv::Mat tvec = (cv::Mat_<double>(3, 1) << seperatedLine[1],
                    seperatedLine[2], seperatedLine[3]);
    cv::Mat rvec = (cv::Mat_<double>(3, 1) << seperatedLine[4],
                    seperatedLine[5], seperatedLine[6]);
...
int main() {
  ...
  for (const auto& d : data) {
    ...
    std::cout << "Position: " << pos.at<double>(0, 0) << " "
              << pos.at<double>(1, 0) << " " << pos.at<double>(2, 0)
              << std::endl;
