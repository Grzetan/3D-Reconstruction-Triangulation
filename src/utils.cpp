...
void loadPointsOneDrone(const char *path,
  ...
  for (const auto &dirEntry : recursive_directory_iterator(path)) {
    ...
    if (directory.find(".csv") != std::string::npos &&
...
std::vector<const tdr::Camera *> loadCamerasXML(const char *path) {
  ...
  if (!result) throw std::runtime_error("Cannot open camera XML file");

  std::vector<const tdr::Camera *> cameras;

  for (pugi::xml_node camera : doc.child("Cameras").children("Camera")) {
    pugi::xml_node controlFrame =
        camera.child("ControlFrames").child("ControlFrame");
    ...
    std::stringstream princialPoint =
        std::stringstream(controlFrame.attribute("PRINCIPAL_POINT").value());
    int width, height;
    ...
    std::stringstream position =
        std::stringstream(controlFrame.attribute("POSITION").value());
    double x, y, z;
    position >> x >> y >> z;
...
void readInputCSV(const char *dir, std::vector<Path> &paths, int frequency,
  ...
  while (std::getline(file, line)) {
    ...
    if (i % frequency != 0 ||
        ((i / frequency < startFrame || i / frequency > endFrame) &&
         startFrame < endFrame)) {
      i++;
      continue;
    };
    ...
    for (int drone = 0; drone < N_DRONES; drone++) {
      ...
      for (int j = 0; j < MARKERS_PER_DRONE; j++) {
        markerPos.push_back(
            {seperatedLine[MARKERS_PER_DRONE * drone * 3 + j * 3 + 1],
             seperatedLine[MARKERS_PER_DRONE * drone * 3 + j * 3 + 2],
             seperatedLine[MARKERS_PER_DRONE * drone * 3 + j * 3 + 3]});
        // markerPos.push_back({seperatedLine[MARKERS_PER_DRONE*drone*3 + j +
        // 1], seperatedLine[MARKERS_PER_DRONE*drone*3 + j + 2],
        // seperatedLine[MARKERS_PER_DRONE*drone*3 + j + 3]});
      }
...
std::vector<cv::Point3d> cross3d(std::vector<cv::Point3d> four_points,
                                 std::vector<double> arms, float err) {
  // znalezienie które ramiona znajdują sie naprzeciwko siebie poprzez
  // sprawdzenie odległości między każdym możliwym dopasowaniem punktów w pary
  std::vector<int> arms_order;

  for (auto j : std::vector<std::vector<int>>{std::vector<int>{1, 2, 3},
                                              std::vector<int>{2, 1, 3},
                                              std::vector<int>{3, 1, 2}}) {
    auto arm1 = four_points[0] - four_points[j[0]];
    auto arm2 = four_points[j[1]] - four_points[j[2]];
    if ((abs(sqrt(arm1.ddot(arm1)) - arms[0]) < err &&
         abs(sqrt(arm2.ddot(arm2)) - arms[1]) < err) ||
        (abs(sqrt(arm1.ddot(arm1)) - arms[1]) < err &&
         abs(sqrt(arm2.ddot(arm2)) - arms[0]) < err)) {
      arms_order = j;
      break;
    }
  }
  ...
  // poniższe obliczenia zostały wyprowadzone z warunku, że iloczyn skalarny
  // między wektorem rozpiętym od punktu środkowego do jednego z ramion, a
  // wektorem od punktu środkowego do końca ramienia prostopadłego powinien
  // wynosić 0
  ...
  double denominator = (delta_collinear.x * delta_collinear.x +
                        delta_collinear.y * delta_collinear.y +
                        delta_collinear.z * delta_collinear.z);
  double t = (delta_collinear.x * delta_non_collinear.x +
              delta_collinear.y * delta_non_collinear.y +
              delta_collinear.z * delta_non_collinear.z) /
             denominator;

  cv::Point3d cross_point = four_points[0] + delta_collinear * t;
  return std::vector<cv::Point3d>{four_points[0], four_points[arms_order[0]],
                                  four_points[arms_order[1]],
                                  four_points[arms_order[2]], cross_point};
}

cv::Point3d convert2global(std::vector<cv::Point3d> cross,
                           cv::Point3d localPoint) {
  cv::Point3d versorX =
      cross[1] - cross[0];  // obliczam wektor kierunkowy osi x
  versorX = versorX / sqrt(versorX.dot(versorX));  // normalizacja
  cv::Point3d versorY =
      cross[3] - cross[2];  // obliczam wektor kierunkowy osi y
  versorY = versorY / sqrt(versorY.dot(versorY));  // normalizacja

  cv::Point3d versorZ =
      versorX.cross(versorY);  // obliczam wersor kierunkowy osi z jako ilczyn
                               // wektorowy wersorów x i y - będzie porostopadły
  // versorZ = versorZ / sqrt(versorZ.dot(versorZ));

  cv::Point3d globalPoint;

  // nowe współrzędne to współrzędne środka lokalnego układu plus lokalne
  // współrzędne punktów przemnożone razy wersory osi lokalnego układu widziane
  // w globalnym układzie
  globalPoint = (cross[4] + localPoint.x * versorX + localPoint.y * versorY +
                 localPoint.z * versorZ);
