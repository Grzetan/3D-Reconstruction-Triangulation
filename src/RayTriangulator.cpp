...
bool RayTriangulator::RayClosestPoint::compute(cv::InputArray params,
                                               cv::OutputArray err,
                                               cv::OutputArray J) const {
  ...
  if (J.needed()) {
    ...
    for (int row = 0; row < Jmat.rows; ++row) {
      ...
      *pJ++ = (distToRay_(rays_[row], {x + epsilon_, y, z}, squared_) -
               distToRay_(rays_[row], {x - epsilon_, y, z}, squared_)) /
              (2 * epsilon_);
      *pJ++ = (distToRay_(rays_[row], {x, y + epsilon_, z}, squared_) -
               distToRay_(rays_[row], {x, y - epsilon_, z}, squared_)) /
              (2 * epsilon_);
      *pJ++ = (distToRay_(rays_[row], {x, y, z + epsilon_}, squared_) -
               distToRay_(rays_[row], {x, y, z - epsilon_}, squared_)) /
              (2 * epsilon_);
    }
  }
...
double RayTriangulator::RayClosestPoint::getError() { return error; }

std::vector<cv::Point3d> RayTriangulator::triangulatePoints(
...
std::pair<cv::Point3d, double> RayTriangulator::triangulatePoint(
    std::vector<CamPointPair> images) {
  std::vector<Ray> rays;
  ...
  cv::Ptr<cv::LMSolver> solver =
      cv::LMSolver::create(solverCompare, MAX_ITERATIONS);
