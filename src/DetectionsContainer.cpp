...
DetectionsContainer::DetectionsContainer(const char* path, int offset,
                                         int recordSize, int startFrame,
                                         int endFrame) {
...
void DetectionsContainer::readFiles(const std::vector<std::string>& files,
                                    int offset, int recordSize, int startFrame,
                                    int endFrame) {
  ...
  if (data.size() < 2)
    throw std::runtime_error("There must be at least 2 cameras");

  for (int i = 1; i < data.size(); i++) {
    if (data[i - 1].size() != data[i].size())
      throw std::runtime_error(
          "Number of frames on all cameras must be the same");
  }
...
int DetectionsContainer::getFrameCount() const { return n_frames; }

int DetectionsContainer::getCamCount() const { return n_cameras; }
...
cv::Point2d DetectionsContainer::getRecord(int camera, int frame,
                                           int detection) const {
...
void DetectionsContainer::addEmptyFrame() {
  for (int i = 0; i < n_cameras; i++) {
    ...
    if (offsetVector.size() > 0) {
      ...
      offsetVector[i].back()[0] =
          0;  // Index=0 means no detection and it doesnt change
    }
  }
}
...
void DetectionsContainer::addDetectionToCamera(cv::Point2d det, int cam,
  ...
  if (originalIndex > -1 && offsetVector.size() > 0)
    offsetVector[cam].back()[data[cam].back().size()] =
        originalIndex + 1;  // +1 becouse index=0 means no detection so all
                            // detections will be shifted
}
...
DetectionsContainer::getDataForTriangulation() {
  ...
  for (int frame = 0; frame < n_frames; frame++) {
    for (int cam = 0; cam < n_cameras; cam++) {
      ...
      if (size > 1) {
        throw std::runtime_error(
            "Function 'getDataForTriangulation' can be used only for one "
            "drone. Each frame can have max one detection");
      } else if (size == 1) {
...
std::vector<int> DetectionsContainer::getOriginalCombination(
    const std::vector<int>& combination, int frame) const {
  if (offsetVector.size() == 0)
    return combination;  // If there is no offset vector the combination stays
                         // as it is
