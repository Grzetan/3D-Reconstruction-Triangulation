...
DroneClassifier::DroneClassifier(Triangulator* triangulator, size_t n_drones)
    : triangulator_(triangulator), n_drones_(n_drones) {
...
bool DroneClassifier::Combination::isCombinationUnique(
    const std::vector<Combination>& combinations) {
  for (const auto& comb : combinations) {
    for (int i = 0; i < combination_.size(); i++) {
      if (combination_[i] == comb.combination_[i] && combination_[i] != 0)
        return false;
    }
  }
  return true;
...
bool DroneClassifier::CombinationPath::operator>(
    const CombinationPath& elem) const {
  return (error < elem.error);
}

void DroneClassifier::classifyDrones(
    const DetectionsContainer& container,
    std::vector<std::vector<cv::Point3d>>& triangulatedPoints) {
  // Create vector of empty frames on each camera to later add [0,0,0] to keep
  // frame count We can't add this point during the algo becouse it would
  // interfere with path classification
  ...
  for (int frame = 0; frame < n_frames; frame++) {
    ...
    for (int n_path = 0; n_path < triangulatedPoints.size(); n_path++) {
      ...
      if (currPath.size() != 0 && currPath.back() != cv::Point3d(0, 0, 0)) {
        Combination bestPointForPath = triangulateWithLastPos(
            currPath.back(), container, usedCombinations, frame);
        if (bestPointForPath.combination_.size() >
            0) {  // If combination was found, add it to used combinations
          processedPaths.push_back(n_path);
          usedCombinations.push_back(bestPointForPath);
          triangulatedPoints[n_path].push_back(
              bestPointForPath.point);  // Add point to path
        }
      }
    }
    ...
    std::vector<Combination> finalCombinations =
        pickBestCombinations(container, frame, usedCombinations);
    classifyPaths(finalCombinations, triangulatedPoints, processedPaths,
                  emptyFrames, frame);
  }
  ...
  for (int i = 0; i < emptyFrames.size(); i++) {
    for (int j = 0; j < emptyFrames[i].size(); j++) {
      triangulatedPoints[i].insert(
          triangulatedPoints[i].begin() + emptyFrames[i][j],
          cv::Point3d(0, 0, 0));
    }
  }
}

void DroneClassifier::fillCombinationQueue(
  ...
  while (iterator.increment()) {
    ...
    int count = std::count_if(combination.begin(), combination.end(),
                              [&](int& i) { return i > 0; });
    if (count < 2) continue;
    ...
    for (int i = 0; i < combination.size(); i++) {
      if (combination[i] <= 0)
        continue;  // Index 0 or -1 means no detection or camera isn't taken
                   // into account so we can skip it
      images.push_back({triangulator_->getCameras()[i],
                        container.getRecord(i, frame, combination[i] - 1)});
    }

    std::pair<cv::Point3d, double> pointWithError =
        triangulator_->triangulatePoint(images);

    // If at some point combination's error is too big, cut the rest of
    // combinations
    if (pointWithError.second > error_) {
      if (!iterator.cut()) break;
    }
    // Add combination to queue only if all of the cameras are taken into
    // account
    else if (std::find(combination.begin(), combination.end(), -1) ==
                 combination.end() &&
             std::count_if(combination.begin(), combination.end(),
                           [&](int& i) { return i > 0; }) >= MIN_CAMERAS) {
      pq.push({container.getOriginalCombination(combination, frame),
               pointWithError.first, pointWithError.second});
    }
  }
}

std::vector<DroneClassifier::Combination> DroneClassifier::pickBestCombinations(
    const DetectionsContainer& container, int frame,
  ...
  while (!combinationsQueue.empty()) {
    ...
    if (c.isCombinationUnique(finalCombinations) &&
        c.isCombinationUnique(usedCombinations) && c.error < error_)
      finalCombinations.push_back(c);
...
DroneClassifier::Combination DroneClassifier::triangulateWithLastPos(
    cv::Point3d pos, const DetectionsContainer& container,
    std::vector<Combination> usedCombinations, int frame) {
  // Create empty container that will later have only one frame with valid
  // detections
  ...
  for (int cam = 0; cam < container.getCamCount(); cam++) {
    for (int det = 0; det < container.detCountForCam(cam, frame); det++) {
      ...
      if (Triangulator::getDistFromRay({triangulator_->getCamera(cam), record},
                                       pos) < MAX_STEP) {
  ...
  while (!combinationsQueue.empty()) {
    Combination c = combinationsQueue.top();
    if (c.isCombinationUnique(usedCombinations) && c.error < error_ &&
        cv::norm(c.point - pos) < MAX_STEP) {
      return c;
    }
    combinationsQueue.pop();
  }
...
bool DroneClassifier::isDetectionInCombinations(
    int cam, int det, const std::vector<Combination>& combinations) {
...
void DroneClassifier::classifyPaths(
    const std::vector<Combination>& finalCombinations,
    std::vector<std::vector<cv::Point3d>>& triangulatedPoints,
    std::vector<int>& processedPaths,
  ...
  for (int i = 0; i < finalCombinations.size(); i++) {
    ...
    for (int j = 0; j < n_drones_; j++) {
      if (std::find(processedPaths.begin(), processedPaths.end(), j) !=
          processedPaths.end())
        continue;

      // Calculate average error from tail of each path
      int nPointsToCheck =
          std::min(triangulatedPoints[j].size(), (size_t)PATH_TAIL);
      if (nPointsToCheck == 0) continue;

      double dist = 0;
      for (int tail = triangulatedPoints[j].size() - nPointsToCheck;
           tail < triangulatedPoints[j].size(); tail++) {
        dist +=
            cv::norm(triangulatedPoints[j][tail] - finalCombinations[i].point);
      }
  ...
  std::sort(combinationPathVec.begin(), combinationPathVec.end(),
            greater<CombinationPath>());

  for (const auto& c : combinationPathVec) {
    // If best path is taken or closest path is a path that doesn't exists
    if (std::find(processedPaths.begin(), processedPaths.end(), c.path) !=
        processedPaths.end()) {
      int emptyPath = -1;
      ...
      if (emptyPath != -1) {
        triangulatedPoints[emptyPath].push_back(
            finalCombinations[c.combination].point);
        processedPaths.push_back(emptyPath);
      }
    } else {
      triangulatedPoints[c.path].push_back(
          finalCombinations[c.combination].point);
  ...
  for (int i = 0; i < n_drones_; i++) {
    if (std::find(processedPaths.begin(), processedPaths.end(), i) ==
        processedPaths.end()) {
