...
namespace tdr {
constexpr auto RAD_TO_DEG =
    57.29577951308232087679;  //!< Radians to degrees factor.
constexpr auto DEG_TO_RAD =
    0.01745329251994329576;  //!< Degrees to radians factor.
...
class Camera {
 ...
 public:
  ...
  void compCxCy() {
    if (width == 0 || height == 0)
      throw std::runtime_error("Set width and height first.");
  ...
  void compFovy() {
    ...
    fovy =
        2.0 *
        atan(tan(fovx * 0.5 * DEG_TO_RAD) / (double(width) / double(height))) *
        RAD_TO_DEG;
  }

  /*! \brief Compute focal length x and focal length y.
   *
   *	Image width, image height, field of view x, and field of view y must be
   *set elier.
   */
  void compFxFy() {
    if (width == 0 || height == 0 || fovx == 0 || fovy == 0)
      throw std::runtime_error("Set width, height, fovx, and fovy first.");
  ...
  void createCamMat() {
    if (fx == 0 || fy == 0 || cx == 0 || cy == 0)
      throw std::runtime_error("Set fx, fy, cx, and cy first.");
    this->cameraMatrix =
  ...
  /*! \brief Compute camera postion based on OpenCV rotation and translation
   * vector.
   *
   */
  void compCamPos() {
   ...
   *
   * Computing parameters: Cx, Cy, field of view y, focal_x, focal_y, camera
   ...
   *
   */
  vector<cv::Point2d> projectPoints(vector<cv::Vec3d> p3d,
                                    bool useDistCoeffs = false) {
    vector<cv::Point2d> p2d;
    cv::Mat tmpDistCoeffs;
    if (useDistCoeffs) tmpDistCoeffs = this->distCoeffs;
  ...
  inline static cv::Mat normalize(cv::Mat quat) {
    ...
    if (normVal < 1.e-6) {
      // throw std::runtime_error("Cannot normalize this quaternion: the norm is
      // too small.");
    }
  ...
  static cv::Mat toRotMatrix(cv::Mat quat) {
    ...
    cv::Matx<double, 3, 3> R{1 - 2 * (c * c + d * d), 2 * (b * c - a * d),
                             2 * (b * d + a * c),     2 * (b * c + a * d),
